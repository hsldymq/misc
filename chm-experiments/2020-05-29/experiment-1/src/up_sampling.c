...
int main(int args, char *argv[]) {
    ...
    filter_node *current_node, *iter_node, *node, *inserted_node;
    ...
    while (iter_node) {
        ...
        if (node->prev) {
            if (node->len != node->prev->len) {
                printf("长度不一致");
                return -1;
            }
            inserted_node = create_node(node->len, node->cap);

            for (iter = 0; iter < node->len; iter++) {
                inserted_node->data[iter] = (node->data[iter] + node->prev->data[iter]) / 2;
            }
            insert_node_after(&target_tabel, inserted_node, node->prev);
        }
